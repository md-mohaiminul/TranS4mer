...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_type='spatial'):
        super().__init__()
        ...
        if attention_type in ['cls', 'joint', 's4']:
            self.aux_norm = norm_layer(dim)
            if attention_type=='s4':
                self.aux_attn = S4(d_model=dim, l_max=950, bidirectional=True, postact='glu', dropout=attn_drop, transposed=True)
            else:
                self.aux_attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
            self.aux_fc = nn.Linear(dim, dim)
            self.aux_drop = DropPath(drop_path) if drop_path > 0. else nn.Identity()

    def forward(self, x, s=None):
        if self.attention_type == 'cls':
            # cls attention
            # [608, 50, 384] -> [32, 19, 50, 384]
            x = einops.rearrange(x, "(b s) l d -> b s l d", s=s)
            aux_res = x[:, :, 0]  # [32, 19, 384]
            aux_res = self.aux_drop(self.aux_attn(self.aux_norm(aux_res)))
            aux_res = self.aux_fc(aux_res)
            x[:, :, 0] = x[:, :, 0] + aux_res  # debug
            x = einops.rearrange(x, "b s l d -> (b s) l d", s=s)

        elif self.attention_type == 'joint':
            x = rearrange(x, "(b s) l d -> b (s l) d", s=s)
            aux_res = self.aux_drop(self.aux_attn(self.aux_norm(x)))
            aux_res = self.aux_fc(aux_res)
            x = x + aux_res
            x = rearrange(x, "b (s l) d -> (b s) l d", s=s)

        elif self.attention_type == 's4':
            x = rearrange(x, "(b s) l d -> b (s l) d", s=s)
            aux_res = self.aux_norm(x)
            aux_res = aux_res.transpose(-1, -2)
            aux_res, _ = self.aux_attn(aux_res)
            aux_res = aux_res.transpose(-1, -2)
            aux_res = self.aux_drop(aux_res)
            aux_res = self.aux_fc(aux_res)
            x = x + aux_res
            x = rearrange(x, "b (s l) d -> (b s) l d", s=s)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block, attention_type = 'spatial'):
        """
        Args:
        ...
        self.blocks = nn.ModuleList([
            block_fn(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                     ...
                     drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, attention_type=attention_type)
            for i in range(depth)])
        ...
        if attention_type == 'cls' or attention_type == 'joint' or attention_type == 's4':
            i = 0
            for m in self.blocks.modules():
                m_str = str(m)
                if 'Block' in m_str:
                    if i > 0:
                        nn.init.constant_(m.aux_fc.weight, 0)
                        nn.init.constant_(m.aux_fc.bias, 0)
                        if attention_type == 'cls' or attention_type == 'joint':
                            nn.init.constant_(m.aux_attn.qkv.weight, 0)
                            nn.init.constant_(m.aux_attn.qkv.bias, 0)
                            nn.init.constant_(m.aux_attn.proj.weight, 0)
                            nn.init.constant_(m.aux_attn.proj.bias, 0)
                        elif attention_type == 's4':
                            nn.init.constant_(m.aux_attn.output_linear[0].weight, 0)
                            nn.init.constant_(m.aux_attn.output_linear[0].bias, 0)
                    i += 1
    ...
    def forward_features(self, x, s=None):
        ...
        # added later
        #W = int(math.sqrt(x.size(1)))
        # x = rearrange(x, "(b k) l d -> b (k l) d", k=2)
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        #added later
        #initialize cls attention weights
        # block.cls_norm.weight.copy_(_n2p(w[f'{block_prefix}LayerNorm_0/scale']))
        # block.cls_norm.bias.copy_(_n2p(w[f'{block_prefix}LayerNorm_0/bias']))
        # block.cls_attn.qkv.weight.copy_(torch.cat([
        #     _n2p(w[f'{mha_prefix}{n}/kernel'], t=False).flatten(1).T for n in ('query', 'key', 'value')]))
        # block.cls_attn.qkv.bias.copy_(torch.cat([
        #     _n2p(w[f'{mha_prefix}{n}/bias'], t=False).reshape(-1) for n in ('query', 'key', 'value')]))
        # block.cls_attn.proj.weight.copy_(_n2p(w[f'{mha_prefix}out/kernel']).flatten(1))
        # block.cls_attn.proj.bias.copy_(_n2p(w[f'{mha_prefix}out/bias']))
